...
export default function CustomerProfilePage() {
  ...
  const handleGenerateDeclarationsPage = async () => {
    if (!customer) return;
    ...
    try {
      ...
      const vehicleLinesForDoc = (customer.vehicles || fallbackVehicleText || "")
        .split(/\r?\n/)
        .map((v) => v.trim())
        .filter(Boolean);

      const discountParts = String(customer.discounts || "")
        .split(/[|,\n]/)
        .map((d) => d.trim())
        .filter(Boolean);
      ...
      const res = await fetch("/api/declarations/generate", {
        ...
        body: JSON.stringify({
          ...
          startDate: customer.renewalDate || "",
          endDate: customer.renewalDate || "",
          totalPremium: customer.monthlyPremium || "",
          vehicles: vehicleLinesForDoc,
          discounts: customer.discounts || "",
      ...
      if (!res.ok) {
        let message = "Error generating declaration";
        try {
          const data = await res.json();
          if (data?.error) {
            message = data.error;
      ...
      a.href = url;
      a.download = `declaration-${customer.name || "customer"}.docx`;
